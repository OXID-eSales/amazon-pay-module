...
- Require `phpseclib/phpseclib: ^3.0.34` in `composer.json`. Shops that installed or updated the module after phpseclib 4.0.0 was released (23 August 2026) and run PHP 8.1 or newer ended up with phpseclib 4.x, whose classes moved to the `phpseclib4\` namespace, while `amzn/amazon-pay-api-sdk-php` still addresses `phpseclib3\Crypt\RSA` when it signs an API request (`Amazon/Pay/API/Client.php`). Every page render then died with `Class "phpseclib3\Crypt\RSA" not found` — not only the checkout pages, because the Amazon sign-in button sits in the service menu and is therefore rendered in the header of every page. The cause is the SDK's own requirement: from version 2.6.8 up to and including the current 2.7.2 it asks for `phpseclib/phpseclib: >=3.0.34` without an upper bound (2.6.4 to 2.6.7 still had a correct `~3.0`), which leaves Composer free to pick the 4.x series. The module now carries the upper bound the SDK is missing; raising our `amzn/amazon-pay-api-sdk-php: ^2.5` constraint would not have helped, because no released SDK version fixes it. Shops that already have phpseclib 4.x installed are moved back to the 3.0.x series by the next `composer update`; a shop that cannot update the module yet can pin the library in the shop root with `composer require phpseclib/phpseclib:"^3.0.34"`. Installations on PHP 7.4 or 8.0, and those that pin `config.platform.php` below 8.1 in the shop's `composer.json`, were never affected: phpseclib 4 requires PHP >= 8.1, so Composer keeps them on the 3.0.x series either way. The same change was made in the OXID 7 module `osc/amazon-pay-module`.
