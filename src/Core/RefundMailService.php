...
class RefundMailService
{
     ...
     *
     * @param Order $order
     * @param float $refundedAmount amount the payment provider confirmed
            ...
            $this->deliver(
                ...
                static function (Email $mailer) use ($order, $refundedAmount, $currency, $recipient): bool {
                    return $recipient === Constants::MAIL_RECIPIENT_OWNER
                        ? $mailer->sendAmazonRefundMailToOwner($order, $refundedAmount, $currency)
                        : $mailer->sendAmazonRefundMailToCustomer($order, $refundedAmount, $currency);
                }
            );
        }
    }
    ...
    public function sendCancelMail(Order $order, ?float $refundedAmount, string $currency): void
    {
        ...
        foreach ($recipients as $recipient) {
            $this->deliver(
                ...
                static function (Email $mailer) use ($order, $refundedAmount, $currency, $recipient): bool {
                    return $recipient === Constants::MAIL_RECIPIENT_OWNER
                        ? $mailer->sendAmazonCancelMailToOwner($order, $refundedAmount, $currency)
                        : $mailer->sendAmazonCancelMailToCustomer($order, $refundedAmount, $currency);
                }
            );
        }
    }
    ...
    protected function log(string $level, string $message, Order $order): void
    {
        try {
            ...
            $logger->log(
                ...
                [
                    'orderId' => $order->getId(),
                    'requestType' => 'amazonpay-refund-mail',
                ]
            );
        } catch (Throwable $throwable) {
