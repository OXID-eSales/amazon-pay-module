...
<div id="content" class="amazonpay-config">
    ...
    <form action="[{$oViewConf->getSelfLink()}]" method="post">
        ...
        <div class="form-group">
            <label for="refundmail">[{oxmultilang ident="OSC_AMAZONPAY_REFUND_MAIL"}]</label>
            <div class="controls">
                <select name="conf[amazonPayRefundMailRecipient]" id="refundmail" class="form-control" required>
                    <option value="0" [{if $config->getRefundMailRecipient() == '0'}]selected[{/if}]>
                        [{oxmultilang ident="OSC_AMAZONPAY_MAIL_RECIPIENT_NONE"}]
                    </option>
                    <option value="1" [{if $config->getRefundMailRecipient() == '1'}]selected[{/if}]>
                        [{oxmultilang ident="OSC_AMAZONPAY_MAIL_RECIPIENT_CUSTOMER"}]
                    </option>
                    <option value="2" [{if $config->getRefundMailRecipient() == '2'}]selected[{/if}]>
                        [{oxmultilang ident="OSC_AMAZONPAY_MAIL_RECIPIENT_OWNER"}]
                    </option>
                    <option value="3" [{if $config->getRefundMailRecipient() == '3'}]selected[{/if}]>
                        [{oxmultilang ident="OSC_AMAZONPAY_MAIL_RECIPIENT_BOTH"}]
                    </option>
                </select>
                <span class="help-block">[{oxmultilang ident="HELP_OSC_AMAZONPAY_REFUND_MAIL"}]</span>
            </div>
        </div>

        <div class="form-group">
            <label for="cancelmail">[{oxmultilang ident="OSC_AMAZONPAY_CANCEL_MAIL"}]</label>
            <div class="controls">
                <select name="conf[amazonPayCancelMailRecipient]" id="cancelmail" class="form-control" required>
                ...
                <span class="help-block">[{oxmultilang ident="HELP_OSC_AMAZONPAY_CANCEL_MAIL"}]</span>
            </div>
        </div>
